...
//  VideoGeneration
//
//  Created by Murphy Brantley on 7/11/17.
...
public class VideoGenerator: NSObject {
    ...
    open func generate(withImages _images: [UIImage], andAudios _audios: [URL], andType _type: VideoGeneratorType, _ progress: @escaping ((Progress) -> Void), success: @escaping ((URL) -> Void), failure: @escaping ((Error) -> Void)) {
        ...
        autoreleasepool {
            ...
            // TODO: Murphy
            // hard code the time for duration to 2 seconds per image
            VideoGenerator.current.audioDurations.removeAll()
            VideoGenerator.current.audioDurations.append(Double(_images.count * 2))
            ...
            var inputSize = VideoGenerator.current.minSize
            var outputSize = VideoGenerator.current.minSize
            if inputSize.width == 0 {
                inputSize = CGSize(width: 1200, height: 1200)
            }
            ...
            if let documentsPath = NSSearchPathForDirectoriesInDomains(.documentDirectory, .userDomainMask, true).first {
                ...
                do {
                    ...
                    print("MURPHY - UNABLE TO REMOVE ITEM")
                }
                            ...
                            videoWriter.finishWriting { () -> Void in
                                if self.audioURLs.isEmpty {
                                    if let path = NSSearchPathForDirectoriesInDomains(.documentDirectory, .userDomainMask, true).first {
                                        ...
                                        var newPath = documentDirectory.appendingPathComponent("\(self.fileName).m4v")
                                        
                                        do {
                                            let fileURLs = try FileManager.default.contentsOfDirectory(at: documentDirectory, includingPropertiesForKeys: nil)
                                            let newURL = newPath.absoluteString.replacingOccurrences(of: "file:///", with: "file:///private/")
                                            newPath = URL(string: newURL)!
                                            ...
                                            failure(error)
                                            print("MURPHY - FAILED TO REMOVE ITEM")
                                        }
    ...
    private func mergeAudio(withVideoURL videoUrl: URL, success: @escaping ((URL) -> Void), failure: @escaping ((Error) -> Void)) {
        ...
        if let videoTrack = videoAsset.tracks(withMediaType: .video).first {
            ...
            if let documentsPath = NSSearchPathForDirectoriesInDomains(.documentDirectory, .userDomainMask, true).first {
                ...
                /// try to start an export session and set the path and file type
                if let exportSession = AVAssetExportSession(asset: mixComposition, presetName: AVAssetExportPresetMediumQuality) {
